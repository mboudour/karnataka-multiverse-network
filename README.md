...
│
├── python/
│   ├── multiverse_pipeline.py             # Main Python multiverse pipeline
│   └── export_edgelist.py                 # Helper: exports .pkl to CSV for R
│
├── r/
│   ├── ergm_multiverse.R                  # ERGM multiverse script (canonical version)
│   └── ergm_multiverse_fast.R             # Lightweight version (m1–m3 only, for testing)
│
├── results/
│   ├── python/
│   │   ├── tables/
│   │   │   ├── regression_multiverse.csv          # All 780 AME estimates
│   │   │   ├── ame_summary.csv / .tex             # Top specifications by AME
...
│   │       └── universe_edges_hist.png            # Edge weight distributions
│   └── r/
│       └── tables/
...
│
├── paper/
│   └── karnataka_multiverse_analysis.pdf  # Compiled paper (workflow draft)
│
└── README.md
```
...
$^\dagger$ **Specifications involving `gwesp`** (geometrically-weighted edgewise shared partners) require full MCMC sampling over a triangle-counting sufficient statistic and are computationally prohibitive at the scale of all 33 villages. These specifications are restricted to the **10 largest villages by household count**: v52 (375), v65 (362), v59 (353), v71 (310), v43 (308), v50 (298), v55 (271), v45 (269), v76 (267), v40 (262).
...
# 3. Run the ERGM multiverse
Rscript r/ergm_multiverse.R data/loan_nomination_edgelist.csv results/r
# Outputs: results/r/tables/
```

> **Note:** The `gwesp` ERGM specifications (m4 directed, m3 undirected) are computationally intensive. On a standard single-core machine, expect several hours of runtime for the top-10 village sample. The lightweight script `r/ergm_multiverse_fast.R` runs only m1–m3 directed and m1–m2 undirected for rapid testing.
...
- **Model choice** (OLS vs. Poisson vs. Negative Binomial) is the single largest driver of AME scale, not the choice of centrality measure.
